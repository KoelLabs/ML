...
feature_dist = panphon.distance.Distance().feature_edit_distance(
    ipa_sequence, second_ipa_sequence
)
...
normalized_by_len = weighted_feature_dist / max(
    len(ipa_sequence), len(second_ipa_sequence)
)
...
print(
    f"Normalized Weighted Feature Edit Distance: {normalized_by_len}"
)  # this does not work...
...
feature_dist = panphon.distance.Distance().feature_edit_distance(
    ipa_sequence, second_noisy_ipa_sequence
)
weighted_feature_dist = panphon.distance.Distance().weighted_feature_edit_distance(
    ipa_sequence, second_noisy_ipa_sequence
)
...
normalized_by_len = weighted_feature_dist / max(
    len(ipa_sequence), len(second_noisy_ipa_sequence)
)
# calculate hamming distance with fastdtw
